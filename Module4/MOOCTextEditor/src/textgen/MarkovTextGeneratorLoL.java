...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText)
	{
		...
		String[] splitText = sourceText.split("[\\s]+");
		starter = splitText[0];
		String prevWord = starter;
		
		for (int i=1; i < splitText.length; i++) {
			ListNode prevWordNode = findWord(prevWord);
			if (prevWordNode == null) {
				ListNode newNode = new ListNode(prevWord);
				newNode.addNextWord(splitText[i]);
				wordList.add(newNode);
			}
			else {
				prevWordNode.addNextWord(splitText[i]);
			}
			prevWord = splitText[i];
		}
		
		String lastWord = splitText[splitText.length-1];
		ListNode prevWordNode = findWord(lastWord);
		if (prevWordNode == null) {
			ListNode newNode = new ListNode(prevWord);
